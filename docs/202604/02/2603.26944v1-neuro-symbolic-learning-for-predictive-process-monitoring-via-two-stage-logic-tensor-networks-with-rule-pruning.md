...
tldr: 本研究针对预测性过程监控中数据驱动方法难以整合领域规则的问题，提出一种基于逻辑张量网络（LTN）的神经符号学习框架。该方法将时序逻辑和一阶逻辑转化为可微约束，并引入创新的两阶段优化策略：首先通过加权公理损失进行预训练以平衡数据与知识，随后利用规则剪枝剔除不一致的公理。实验证明，该方法在提升预测准确性的同时确保了业务合规性，特别是在小样本约束场景下优势显著。
selection_source: fresh_fetch
motivation: 传统数据驱动模型在处理序列事件时，无法有效利用领域特定的时序约束和逻辑规则，限制了预测精度和合规性。
method: 提出一种集成逻辑张量网络（LTN）的神经符号方法，采用加权公理损失预训练和基于满意度动态的规则剪枝两阶段优化策略。
result: 在四个真实世界数据集上的评估显示，该方法显著提升了预测性能，且在合规性约束强、训练样本有限的情况下表现尤为出色。
conclusion: 通过两阶段优化将领域知识注入深度学习模型，不仅能提高预测准确率，还能确保模型输出符合业务逻辑约束。
---

## 摘要
对序列事件数据进行预测建模对于欺诈检测和医疗监控至关重要。现有的数据驱动方法从历史数据中学习相关性，但未能纳入特定领域的序列约束和管理事件关系的逻辑规则，从而限制了准确性和合规性。例如，医疗程序必须遵循特定的顺序，而金融交易必须遵守合规规则。我们提出了一种神经符号方法，利用逻辑张量网络（LTNs）将领域知识集成为可微逻辑约束。我们使用线性时序逻辑和一阶逻辑对控制流、时序和有效载荷知识进行形式化。我们的核心贡献是一种两阶段优化策略，旨在解决 LTNs 倾向于以牺牲预测准确性为代价来满足逻辑公式的问题。该方法在预训练期间使用加权公理损失来优先进行数据学习，随后根据满足度动态进行规则剪枝，仅保留一致且有贡献的公理。对四个真实世界事件日志的评估表明，领域知识注入显著提高了预测性能，而两阶段优化被证明是至关重要的（如果没有它，知识注入可能会严重降低性能）。该方法在合规样本有限的受限合规场景中表现尤为出色，在确保遵守领域约束的同时，实现了优于纯数据驱动基准模型的性能。
