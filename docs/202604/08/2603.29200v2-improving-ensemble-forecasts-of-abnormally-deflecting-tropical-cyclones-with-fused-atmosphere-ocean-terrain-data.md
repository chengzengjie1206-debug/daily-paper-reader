...
title_zh: 利用融合的大气-海洋-地形数据改进异常偏转热带气旋的集合预报
authors: "Qixiang Li, Yuan Zhou, Shuwei Huo, Chong Wang, Xiaofeng Li"
date: 2026-03-31
pdf: "https://arxiv.org/pdf/2603.29200v2"
tags: ["query:ts"]
score: 6.0
evidence: 基于深度学习的热带气旋路径预测
...
## 摘要
基于深度学习的热带气旋（TC）预报方法展现出巨大的潜力和应用优势，因为与数值天气预报模型相比，它们具有更低的计算成本和更快的运行速度。然而，现有的深度学习方法仍存在关键局限性：它们只能处理单一类型的序列轨迹数据或同质气象变量，无法实现对异常偏转热带气旋的准确预报。为了应对这些挑战，我们提出了两项突破性贡献。首先，我们构建了一个名为 AOT-TCs 的多模态、多源数据集，用于西北太平洋盆地的热带气旋预报。作为同类数据集中的首个，它创新性地整合了来自大气、海洋和陆地的异构变量，从而获得了一个全面且信息丰富的气象数据集。其次，基于 AOT-TCs 数据集，我们提出了一种能够处理正常和异常偏转热带气旋的预报模型。这是首个采用显式大气-海洋-地形耦合架构的热带气旋预报模型，使其能够有效捕捉跨物理领域的复杂相互作用。对 2017 年至 2024 年西北太平洋所有热带气旋个例进行的广泛实验表明，我们的模型在热带气旋预报方面达到了最先进的性能：它不仅显著提高了正常热带气旋的预报精度，还突破了异常偏转热带气旋预报的技术瓶颈。
