...
tldr: 针对时间序列基础模型（TSFM）在多领域训练中面临的严重异构性和梯度冲突问题，本文提出了一种双层异构联邦学习方法。该方法通过局部正则化减少域内冲突，并利用域感知聚合增强跨域协作，从而在提取通用知识的同时降低跨域干扰。实验证明，该方法在点预测、概率预测及零样本任务上均优于现有的集中式和联邦式基准，为在异构环境下从头训练TSFM提供了高效且灵活的新路径。
selection_source: fresh_fetch
motivation: 现有时间序列基础模型在混合大规模异构数据训练时，常因领域间的梯度冲突导致模型表示能力受损。
method: 设计了一种双层联邦学习框架，通过局部正则化约束域内一致性，并利用域感知聚合机制优化跨域协作。
result: 实验显示该方法在多样化的预测任务和零样本场景下均取得了优于集中式训练和传统联邦学习基准的性能。
conclusion: 该研究证明了通过精细化处理双层异构性，可以有效提升时间序列基础模型在复杂异构环境下的泛化能力和表示质量。
---

## 摘要
时间序列数据中的异构性比视觉或语言数据更为显著，因为时间动态在不同领域和任务之间存在巨大差异。现有的从头开始训练时间序列基础模型（TSFMs）的工作通常采用混合批次策略，合并大规模数据集，这可能导致梯度冲突并降低表示质量。为解决这一问题，我们提出了一种细粒度学习方法，在减少跨域干扰的同时，从异构序列中提取不变知识。我们从两个层面刻画异构性：域间（inter-domain）和域内（intra-domain）。为了应对这种双层异构性，我们设计了一种联邦学习方法，通过局部正则化强制实现域不变且语义一致的表示，从而缓解域内冲突；并通过域感知聚合增强跨域协作，从而解决域间差异。在不同基准测试上的实验表明，使用我们方法训练的 TSFMs 在点预测和概率预测方面始终优于集中式和联邦式 TSFM 基准模型，同时在大规模场景下实现了具有竞争力的零样本性能，为在异构环境中从头开始训练 TSFMs 提供了一条灵活的路径。
