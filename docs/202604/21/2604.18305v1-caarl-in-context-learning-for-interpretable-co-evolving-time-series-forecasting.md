...
title_zh: CAARL：用于可解释协同演化时间序列预测的情境学习
authors: "Etienne Tajeuna, Patrick Asante Owusu, Armelle Brun, Shengrui Wang"
date: 2026-04-20
pdf: "https://arxiv.org/pdf/2604.18305v1"
tags: ["query:ts"]
score: 8.0
evidence: 可解释的协同演化时序预测
tldr: 针对具有复杂依赖和非平稳动态的协同演化时间序列预测，本文提出CAARL框架。该方法利用大语言模型（LLM）的上下文学习能力，将时间序列分解为自回归片段并构建时间依赖图，随后将其序列化为叙述性文本。通过类思维链的推理路径，CAARL在保持高预测精度的同时，显著提升了预测过程的可解释性，为复杂时间序列分析提供了新思路。
selection_source: fresh_fetch
motivation: 旨在解决协同演化时间序列中复杂的非平稳动态难以建模以及预测过程缺乏可解释性的挑战。
method: 将时间序列分解为自回归段并构建时间依赖图，将其序列化为叙述性文本，利用LLM进行类思维链的推理与预测。
result: 在真实世界数据集上的实验验证了CAARL的有效性，其预测精度可媲美最先进的方法，并能提供透明的推理过程。
conclusion: CAARL通过结合LLM的上下文学习与结构化时间建模，实现了兼具竞争力和高度可解释性的时间序列预测。
---

## 摘要
在本文中，我们利用大语言模型（LLM）方法研究了具有复杂依赖关系和非平稳动态特性的协同演化时间序列预测。我们提出了一种名为上下文感知 ARLLM（CAARL）的新型建模方法，该方法提供了一个可解释的框架，用于解码影响协同演化序列变化的上下文动态。CAARL 将时间序列分解为自回归片段，构建时间依赖图，并将该图序列化为叙述文本，以便由 LLM 进行处理。这种设计产生了一种类似于思维链（Chain-of-Thought）的推理路径，其中中间步骤捕捉上下文动态，并以透明的方式引导预测。通过将预测与显式推理轨迹相结合，CAARL 在保持准确性的同时增强了可解释性。在真实世界数据集上的实验验证了其有效性，使 CAARL 成为现有最先进预测方法的一个具有竞争力的且可解释的替代方案。
