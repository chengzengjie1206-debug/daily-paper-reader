...
tldr: 针对终端区空中交通流量预测中现有时间序列方法忽略微观状态信息的问题，本文提出AeroSense框架。该框架摒弃了传统的宏观流量聚合模式，直接将实时空域态势建模为动态飞机状态集。通过引入态势感知状态表示、掩码自注意力机制及解耦预测头，AeroSense能直接从微观运动学数据中捕捉交互并预测流量。实验证明其在预测精度、高峰期鲁棒性及可解释性方面均优于现有基准。
selection_source: fresh_fetch
motivation: 传统基于时间序列的预测方法将轨迹聚合为宏观序列，忽略了飞机的实时运动学特征及与空域边界的相对位置等关键微观信息。
method: 提出一种直接从状态到流量的建模框架，利用态势感知表示和掩码自注意力机制处理变长飞机状态集，并采用解耦预测头捕捉不同功能区的流量动力学。
result: 在大规模真实数据集上的实验显示，AeroSense在各项指标上均达到SOTA水平，且在交通高峰时段表现出卓越的鲁棒性和帕累托最优性能。
conclusion: 研究证明直接建模微观飞机状态比传统时间序列方法具有更高的预测保真度，并能通过注意力机制提供有意义的可解释性。
---
