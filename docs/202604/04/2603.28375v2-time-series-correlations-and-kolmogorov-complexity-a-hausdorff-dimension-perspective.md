...
tldr: 本研究针对时间序列分析中常见的伪相关问题，提出利用柯尔莫哥洛夫复杂度作为防范虚假正态的手段。通过有效豪斯多夫维度理论，证明了独立序列间的偶然相关概率随其复杂度增加而呈指数级衰减。研究引入了联合复杂度指标 J_LZ，并通过逻辑斯谛映射和分数布朗运动模型验证了其在识别低复杂度伪相关及预测同步化方面的有效性，为提升相关性分析的可靠性提供了新视角。
selection_source: fresh_fetch
motivation: 旨在解决时间序列分析中低复杂度模式容易导致伪相关并产生虚假正态结论的问题。
method: 基于有效豪斯多夫维度理论分析复杂度与相关性的关系，并提出联合复杂度指标 J_LZ 来衡量序列对的整体复杂度。
result: 发现偶然相关概率随复杂度指数级下降，且 J_LZ 指标能有效区分真实相关与低复杂度引起的伪相关。
conclusion: 建议在进行相关性分析时应同时报告 J_LZ 指标，并对低复杂度序列之间的高相关性结果保持审慎态度。
---

## 摘要
伪相关在时间序列分析中十分常见，因为简单的低复杂度模式即使在不相关的序列之间也能产生较高的皮尔逊相关性。我们认为，被解释为抗压缩性的柯尔莫哥洛夫复杂度为防止此类假阳性提供了原则性的保障。利用有效豪斯多夫维数，我们证明了两个独立序列之间发生偶然相关的概率随其复杂度的增加而呈指数级衰减，而噪声会虚增观测到的复杂度，因此在实践中必须予以考虑。我们通过耦合逻辑斯谛映射和多元分数布朗运动（mfBm）阐述了这些观点，其中赫斯特参数 \(H\) 同时控制复杂度与豪斯多夫维数 \((\dim_H = 2 - H)\)。两种模型均表明，假阳性在低复杂度序列中比在高复杂度序列中更为常见。我们引入了联合复杂度指标 \[ J_{\rm LZ} = \sqrt{\widetilde{C}_{\rm LZ}(x)\widetilde{C}_{\rm LZ}(y)} \]，该指标捕捉的是联合高复杂度，而非个体复杂度之间的简单相似性。其阈值可以根据 mfBm 的假阳性曲线进行校准。在逻辑斯谛映射中，\(J_{\rm LZ}\) 还能在同步发生前预见个体复杂度的崩溃。我们建议首先确立平稳性，然后在报告相关系数 \(\rho\) 的同时报告 \(J_{\rm LZ}\)，并对低复杂度序列之间的高相关性保持怀疑态度。
