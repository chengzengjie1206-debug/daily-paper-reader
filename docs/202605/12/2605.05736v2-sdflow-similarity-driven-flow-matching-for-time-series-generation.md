...
tags: ["query:ts"]
score: 7.0
evidence: 长周期生成与时间序列建模
tldr: SDFlow 针对时间序列生成中自回归模型存在的曝光偏差问题，提出了一种基于流匹配（Flow Matching）的非自回归框架。该方法在预训练的向量量化（VQ）潜空间中运行，通过低秩流形分解和类别后验引导，实现了高效的并行序列生成。实验证明，SDFlow 在长序列生成质量和推理速度上均显著优于现有自回归基准，达到了领域领先水平。
selection_source: fresh_fetch
motivation: 传统的自回归时间序列生成模型受限于曝光偏差，导致长序列生成时误差累积且推理效率低下。
method: 提出在 VQ 潜空间中利用流匹配进行全局传输，并结合低秩流形分解与类别后验分布来处理高维离散特征。
result: 在多个数据集上取得了 SOTA 性能，显著降低了 Context-FID 指标，并大幅提升了推理速度。
conclusion: SDFlow 证明了非自回归流匹配在处理复杂时间序列生成任务中的优越性，兼顾了生成质量与计算效率。
---
