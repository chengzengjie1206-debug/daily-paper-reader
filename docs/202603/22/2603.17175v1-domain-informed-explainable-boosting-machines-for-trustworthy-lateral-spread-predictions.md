...
## 摘要
可解释提升机 (EBMs) 通过加性形状函数提供透明的预测，从而能够直接检查特征贡献。然而，EBMs 可能会学习到非物理关系，从而降低其在自然灾害应用中的可靠性。本研究提出了一种领域知识引导的框架，旨在提高用于侧向位移预测的 EBMs 的物理一致性。我们的方法基于领域知识修改学习到的形状函数。这些修改在纠正非物理行为的同时，保留了数据驱动的模式。我们将该方法应用于 2011 年克赖斯特彻奇地震数据集，并纠正了原始 EBM 中观察到的非物理趋势。所得模型产生了更具物理一致性的全局和局部解释，且在准确性方面仅有较小的折衷（4-5%）。
