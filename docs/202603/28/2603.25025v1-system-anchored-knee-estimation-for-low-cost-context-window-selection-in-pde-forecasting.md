...
title_zh: 用于偏微分方程预测中低成本上下文窗口选择的系统锚定拐点估计
authors: "Wenshuo Wang, Fan Zhang"
date: 2026-03-26
pdf: "https://arxiv.org/pdf/2603.25025v1"
tags: ["query:ts"]
score: 7.0
evidence: 基于PDE的时序预测上下文窗口选择方法
...
## 摘要
自回归神经偏微分方程（PDE）模拟器根据有限的历史数据逐步预测物理场的演化，但此类模拟器的低成本上下文窗口选择仍是一个尚未形式化的问题。现有的时间序列预测上下文窗口选择方法包括穷举验证、直接低成本搜索和系统论记忆估计，但它们要么成本高昂、脆弱，要么与下游的外推（rollout）性能不直接对齐。我们将固定窗口自回归神经 PDE 模拟器的显式上下文窗口选择形式化为一个独立的低成本算法问题，并提出了系统锚定拐点估计（SAKE）。这是一种两阶段方法，首先从具有物理可解释性的系统锚点中识别出一个小型结构化候选集，然后在其中进行感知拐点的下游选择。在共享的 L ∈ {1, ..., 16} 协议下评估的所有八个 PDEBench 族中，SAKE 是评估方法中整体表现最强的预算匹配低成本选择器，实现了 67.8% 的精确匹配（Exact）、91.7% 的 Within-1、6.1% 的平均拐点遗憾值（mean regret@knee）以及 0.051 的成本比（节省了 94.9% 的归一化搜索成本）。
