...
tldr: 针对工业设备预测性维护中传感器数据存在的多尺度时间相关性及训练数据稀缺等挑战，本文提出了一种轻量化的多尺度Transformer模型MsFormer。该模型通过多尺度采样模块和定制的编码机制捕捉复杂序列依赖，并采用基于池化操作的轻量化注意力机制替代传统自注意力，以提升在小样本环境下的鲁棒性。实验证明，MsFormer在多个真实数据集上表现优异，具有极强的泛化能力和高服务质量。
selection_source: fresh_fetch
motivation: 现有深度学习模型难以有效处理工业物联网数据中的多尺度时间相关性，且在数据稀缺环境下部署困难。
method: 提出MsFormer框架，利用多尺度采样模块捕捉序列相关性，并结合轻量化池化注意力机制降低模型复杂度。
result: 在真实世界数据集上的广泛实验表明，MsFormer在预测精度和跨设备泛化性能上均显著超越了现有最先进方法。
conclusion: MsFormer为工业预测性维护提供了一个高效、鲁棒的统一AI服务模型，能够适应复杂的工业运行环境。
---
