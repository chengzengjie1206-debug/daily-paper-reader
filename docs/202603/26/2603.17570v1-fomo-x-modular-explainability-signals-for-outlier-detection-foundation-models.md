...
tldr: 针对表格基础模型（如PFN）在异常检测中缺乏可解释性的问题，本文提出FoMo-X框架。该框架通过在预训练模型嵌入层添加轻量级辅助诊断头，利用离线模拟训练将复杂的解释性指标（如不确定性和严重程度）转化为单次推理。实验证明，FoMo-X在保持零样本检测性能的同时，以极低开销提供了高保真度的解释信号，增强了模型的可靠性。
selection_source: fresh_fetch
motivation: 现有的异常检测基础模型虽具备强大的零样本能力，但其黑盒特性和高昂的解释计算成本限制了在安全关键领域的应用。
method: 提出FoMo-X框架，通过在冻结的PFN骨干网络上附加辅助诊断头，将复杂的后验解释属性蒸馏为高效的单次推理。
result: 在ADBench等基准测试上，FoMo-X能以极低的推理开销准确还原真实诊断信号，并提供校准后的置信度度量。
conclusion: FoMo-X成功弥合了基础模型性能与可解释性之间的鸿沟，为构建可信、高效的零样本异常检测系统提供了可行方案。
---
