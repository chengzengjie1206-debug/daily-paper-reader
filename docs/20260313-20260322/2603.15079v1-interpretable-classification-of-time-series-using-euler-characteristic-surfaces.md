...
conclusion: ECS是一种高效、稳定且具有高度可解释性的时间序列拓扑特征表示方法，在生物医学信号处理领域具有显著优势。
---

## 摘要
持续同调（Persistent homology, PH）是拓扑数据分析中的传统方法，其计算成本高昂，在应用机器学习（ML）之前需要对其特征进行进一步向量化，且仅能捕获沿空间轴的信息。针对时间序列数据，我们提出欧拉示性数曲面（Euler Characteristic Surfaces, ECS）作为一种基于欧拉示性数（$χ$，一种基本的拓扑不变量）的替代拓扑特征。ECS 提供了一种计算高效、时空且固有离散化的特征表示，可直接作为机器学习模型的输入。
